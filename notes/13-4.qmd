...
```{r}
#| fig.cap: "Taking many gradient steps leads us towards an image that optimizes a neuron's activation."
#| echo: FALSE
...
```{r}
#| fig.cap: "The hypothetical images that maximize the activations for 40 different neurons. These neurons seem to pull out features related to color and edge orientations."
squash <- function(x) {
  (x - min(x)) / (max(x) - min(x))
}
...
#| fig.show: "hold"
#| preview: TRUE
#| fig.cap: "The results of the corresponding optimization for 40 neurons in layer 8."
par(mfrow = c(5, 8), mai = rep(0.00, 4))
