...
```{r}
#| fig.heigt: 4
#| fig.cap: "A faceted barplot view of the original topic distributions
#| with only very limited filtering."
...
```{r}
#| fig-height: 2
#| fig.cap: "An equivalent heatmap view of the above faceted barplot."
topics %>%
  filter(beta > 3e-4) %>%
...
```{r}
#| preview: TRUE
#| fig.cap: "The top words associated with the four fitted topics from the Great Library Heist example."
top_terms <- topics %>%
  group_by(topic) %>%
  slice_max(beta, n = 10) %>%
...
```{r}
#| fig.cap: "An illustration of the formula used for computing a word's discrimination between topics. The value of D is large when topic k has much larger probability than topic l."
...
#| fig-height: 7
#| fig-width: 3
#| fig.cap: "A heatmap of the terms that are most discriminative across the four topics."
...
```{r}
#| fig.cap: "A boxplot of the document memberships. It seems that most documents are definitively assigned to one of the four topics."
...
#| fig-height: 4
#| fig-width: 6
#| fig.cap: "A jittered scatterplot of the topic memberships associated with each document."
...
#| fig-height: 10
#| fig-width: 4
#| fig.cap: "A structure plot view of each chapter's topic memberships."
gamma <- memberships %>%
