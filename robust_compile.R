...
skip_files <- c("_posts/2024-12-27-week07-01/week07-01.Rmd")
remaining_files <- setdiff(remaining_files, skip_files)
cat("Skipping", length(skip_files), "problematic files, processing", length(remaining_files), "files\n")
...
for (i in seq_along(remaining_files)) {
  ...
  tryCatch({
    ...
    error_files <<- c(error_files, remaining_files[i])
